...
# Mode selection
mode = st.radio("Select Mode:", ["Patient-Specific", "General"],
                horizontal=True,
                key="mode")
...
# Clear patient lock when switching modes
...
if mode == "Patient-Specific":
    # Load roster only in patient mode
    ...
    st.markdown("### General Document Search")
    st.info(
        "Ask questions about general medical documents (not patient-specific)."
    )
